...
{#if workflows && workflows.length > 0}
  <div class="mt-6 flex flex-wrap gap-6">
    {#each workflows as workflow}
      <a
        href="/commands/{command.slug}/{workflow.slug}"
        class="card min-w-[280px] bg-base-200 p-6 transition-all hover:scale-105 hover:bg-base-300"
      >
          ...
          <div class="flex-1">
            ...
            <div class="mt-2 flex flex-col gap-2">
              {#each workflow.inputs as input}
                <span class="badge badge-primary text-xs">{input.name}: {input.type}</span>
              {/each}
            </div>
          </div>
          ...
          <div class="flex-1">
            ...
            <div class="mt-2 flex flex-col gap-2">
              {#each workflow.outputs as output}
                <span class="badge badge-primary text-xs">{output.name}: {output.type}</span>
              {/each}
            </div>
          </div>
        </div>
