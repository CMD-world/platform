...
export async function validateSessionToken(token: string, cookies: Cookies): Promise<SessionValidationResult> {
...
type GitHub = { githubId: string };
type Method = Password | Google | GitHub;
...
export async function register(email: string, method: Method): Promise<void> {
  ...
  } else if ("githubId" in method) {
    await db.insert(userTable).values({ email, data: method.githubId, method: "github" });
  }
}
...
export async function login(email: string, method: Method, cookies: Cookies) {
  ...
  if ("password" in method && !(await verifyPassword(method.password, user.data))) {
    ...
    throw new Error("E-mail or password is invalid");
  } else if ("githubId" in method && user.data != method.githubId) {
    throw new Error("E-mail or password is invalid");
  }
